...
public sealed class SetWorkingDayHandler : ICommandHandler<SetWorkingDayCommand>
{
    ...
    public SetWorkingDayHandler(IStoreCalendarRepository repo, StoreCalendarService service, ICurrentUser currentUser, IUnitOfWork uow)
    {
        _repo = repo;
        _service = service;
    ...
    public async Task Handle(SetWorkingDayCommand command, CancellationToken ct)
    {
        ...
        _service.SetWorkingDay(calendar, staff, _currentUser.UserId, command.WorkingDay);
