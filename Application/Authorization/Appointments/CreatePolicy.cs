...
public sealed class CreatePolicy : ICreatePolicy
{
    private readonly IRequestContext _context;
    private readonly IStaffRepository _repo;

    public CreatePolicy(IRequestContext context, IStaffRepository repo)
    {
        _context = context;
        _repo = repo;
    }

    public async Task EnsureCanCreateAsync(ScheduleCommand command, CancellationToken ct)
    {
        if (_context.IsUser)
        {
    ...
    private void EnsureUserCanCreate(ScheduleCommand command)
    {
    ...
    private async Task EnsureProfessionalCanCreate(ScheduleCommand command, CancellationToken ct)
    {
